...
xCol = [item for item in data.columns if item not in ['OBJECT','lon','lat','Class']]
X = data[xCol]
y = data['Class']
...
# X, y, GeoID= data_raw() 对应的全区域的X,y
# y_pred_gdbt = gdbt_model.predict(X)
# y_pred_proba_gdbt = gdbt_model.predict_proba(X)[:,1] 输出概率
